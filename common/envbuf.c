...
{
	if (envp == NULL) return NULL; //opa said don't comment this
...
void envbuf_free(char*const envp[])
{
	if (envp == NULL) return;
...
void envbuf_setenv(char **envpp[], const char *name, const char *value, int override)
{
	if (envpp) {
		...
		int existingEnvIndex = envbuf_find(envp, name);
